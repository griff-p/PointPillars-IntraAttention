...
def main(args):
    ...
    train_dataloader = get_dataloader(
        dataset=train_dataset, batch_size=args.batch_size, num_workers=args.num_workers, shuffle=True
    )
    val_dataloader = get_dataloader(
        dataset=val_dataset, batch_size=args.batch_size, num_workers=args.num_workers, shuffle=False
    )
        ...
        for i, data_dict in enumerate(tqdm(train_dataloader)):
            if not args.no_cuda:
                for key in data_dict:
                    data_dict[key] = [item.cuda() for item in data_dict[key] if torch.is_tensor(item)]
            ...
            # Handle outputs as tuples if necessary
            ...
            if isinstance(outputs, tuple):
                bbox_cls_pred, bbox_pred, bbox_dir_cls_pred, anchor_target_dict = outputs
            else:
                bbox_cls_pred, bbox_pred, bbox_dir_cls_pred, anchor_target_dict = outputs, None, None, None
